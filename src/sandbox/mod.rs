...
use crate::receipts::emit_capability_event;
...
impl Sandbox {
    ...
    /// the sandbox with registered host functions. Fail-closed on any error.
...
///
/// **TOCTOU limitation**: `stat()` then `read()` has a race window.
/// Single-threaded Phase 1 scope; Phase 2/3 addresses with `openat` +
/// fd passing or immutable roots.
fn aegis_fs_read(
    ...
    // Order matters: if OOB check (S-5) or path traversal (S-2/S-3) or size limit (S-4)
    // ran before this, we'd emit no receipt for those violation branches.
    // This ensures emit_capability_event has the capability name for ALL 5 scenarios.
    let cap = caller
        .data()
        .capabilities
        .iter()
    ...
    let path_bytes = memory
        ...
        .ok_or_else(|| {
            // Emit capability event for guest memory OOB (S-5, REQ-107)
            emit_capability_event(&cap_name, "", 0, "Trap");
    ...
    // Resolve the guest path relative to allowed_root, then canonicalize
    ...
    // 7. Emit receipt stub BEFORE read (captures S-2, S-3, S-5)

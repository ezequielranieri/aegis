...
### Decision
mTLS is configurable via `server.tls` in `RuntimeConfig`: when `server.tls` is present, a client certificate is mandatory with CN/SAN validated against `expected_identity` (via `AegisClientCertVerifier`); when absent, the server starts without TLS with a warning (not suitable for production).
...
- **Fail-closed when TLS is enabled**: the fail-closed mode exists when `server.tls` is configured — `client_auth_mandatory()` returns `true` and connections without a valid client certificate are rejected with `UNAUTHENTICATED`. Without `server.tls`, there is no encryption at all and startup emits a warning. mTLS is optional to configure, but when TLS is enabled client authentication is mandatory — there is no server-only TLS mode.
...
### Consequences
- Every `aegis-runtime` deployment with mTLS enabled requires CA cert, server cert/key, and client cert/key configured in `RuntimeConfig`
